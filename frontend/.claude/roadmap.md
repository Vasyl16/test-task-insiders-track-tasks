...
- V2/V3 Workspace + Project UI (done — create/list/view/edit/delete, plus a full visual redesign; no member-invitation UI yet)
...
- Workspace detail page (`/workspaces/:workspaceId`): name/description, edit (owner-only) and delete (owner-only), back link to dashboard, its projects as numbered ledger rows
- Create-project form (in a modal); edit and delete a project (creator-or-owner for both — the project detail page's header and the workspace page's project row each have their own `Edit`/`Delete` pair)
- Client-side gating of destructive/edit actions based on `ownerId`/`createdBy` (the backend remains the actual enforcement)
...
- Drag-and-drop migrated from native HTML5 DnD to DnD Kit (`@dnd-kit/core`) per explicit request — same visual/behavioral result (drag between columns, same-column drop is a no-op, same `updateTask` mutation), but adds touch support and keyboard-driven dragging (`KeyboardSensor`) for free; see `architecture.md`'s "Drag-and-drop: DnD Kit" section
