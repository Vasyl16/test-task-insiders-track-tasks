...
- `socket.io-client` — real-time Kanban board sync (task create/update/delete from other users/tabs), one app-wide singleton in `shared/api/socket/socketClient.ts`
...
- `shared/api/queries`: one file per domain (auth, workspace, project, task, invite). Plain functions that call the API through the shared Axios instance — no React or UI concerns, and no query/mutation split (a domain file holds both reads and writes, e.g. `getMe`, `login`, `register`, `logoutRequest`; `getWorkspaces`, `createWorkspace`, `getWorkspaceMembers`, ...).
- `shared/api/services`: React Query hooks per domain (`useAuth`, `useLogin`, `useRegister`; `useWorkspaces`/`useWorkspace`/`useWorkspaceMembers`/`useCreateWorkspace`/`useUpdateWorkspace`/`useDeleteWorkspace`; `useProjects`/`useProject`/...; `useTasks`/`useTask`/`useCreateTask`/`useUpdateTask`/`useDeleteTask`; `useMyInvites`/`useCreateInvite`/`useAcceptInvite`/`useDeclineInvite`) that wrap `queries` for use in components. `useProjectRealtime(workspaceId, projectId)` (called from `TaskBoard`) joins that project's WebSocket room and invalidates `tasks.lists(...)` when another user/tab creates, updates, or deletes a task.
- `shared/api/socket/socketClient.ts`: the one app-wide `Socket.IO` client singleton (`autoConnect: false`, connected/disconnected by `RealtimeConnectionManager`). Its `auth` option is a callback (re-reads the current access token from `tokenManager` on every reconnect), not a static object captured once — the token rotates independently of the socket's lifetime.
...
- `shared/ui`: presentation-only primitives with no domain logic — `Button` (`variant: 'primary' | 'ghost' | 'logout' | 'nav'`), `Input`, `Listbox` (fully custom dropdown — colored dot per option, custom panel — for when a native `<select>`'s option styling isn't enough; wire it up via React Hook Form's `Controller`, not `register()`), `FormError`, `Modal` (portal-rendered dialog, Escape/backdrop to close), `ListFilterBar` (search + sort + mine-vs-not-mine controls, shared by the Dashboard's workspace list and a Workspace's project list — both are name/createdAt/owner-shaped paginated lists). Used by `features/auth`/`features/workspace`/`features/project`/`features/task`'s forms, `widgets/app-header`, and `pages/dashboard`/`pages/workspace`. See "Visual Design System" in `architecture.md` for the token system these are built on.
- `shared/hooks`: generic reusable hooks not tied to a domain — `useDebouncedValue` (delays reflecting a fast-changing value, e.g. a search input, so a query keyed off it doesn't fire once per keystroke). `shared/utils`, `shared/constants`: no business logic; still empty, created as content actually exists.
...
Not every folder needs to exist before it has content — create a folder when the first file that belongs in it is added, not preemptively. `shared/hooks` now has `useDebouncedValue.ts`; `shared/utils`, `shared/constants` are still intentionally absent from the current tree.
...
- `logout`'s `onSuccess` (`shared/api/services/useAuth.ts`) must call `queryClient.setQueryData(queryKeys.auth.me, null)` **before** `queryClient.clear()`, not after — `clear()` first silently breaks the redirect (it destroys the `auth.me` cache entry before the explicit write can notify the observer, same "removeQueries doesn't notify, setQueryData does" issue). The `clear()` itself is required, not optional: the `QueryClient` is one in-memory instance for the whole SPA session, so without it every other cached query (workspaces, projects, tasks, invites, ...) survives a logout and gets served — stale, and belonging to the wrong account — to whoever logs in next in that same tab. Confirmed via a real browser with two accounts in one session.
